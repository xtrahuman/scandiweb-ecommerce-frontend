...
class Category extends React.Component {
  ...
  AddToCart = (attributes, gallery, index) => {
    const {
      myItem, addAttrib, updateCart, addToCart, allCart,getProduct
    } = this.props;
    const priceValue = 10.00;
    handleAddLogic(myItem, addAttrib, updateCart, addToCart, allCart, priceValue, attributes, gallery)
    const allQuickShop = document.querySelectorAll('.quick-shop')
    const imgOverlay = document.querySelectorAll('.img-overlay')
    allQuickShop[index].classList.remove('open-shop')
    imgOverlay[index].classList.remove('opaque-shop')
                  ...
                  <div className="d-flex row">
                    {data.category.products.map(({
                      ...
                      <div className="button-contain">
                        <button onClick={() => this.AddToCart(attributes, gallery, index)} disabled={false} className="details-button quick-shop-btn" type="button">ADD</button>
                      </div>
                            </div>
                            <div className={`${inStock ? '' : 'img-opacity'} category-image-contain`} >
                              ...
                              <div role='button' onMouseDown={()=> this.toggleQuickShop(index, attributes)} className="add-cart-logo" ><img src={addCartLogo} alt="addCart" /></div>
                            </div>
                          </div>
